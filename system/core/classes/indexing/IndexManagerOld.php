...
final class IndexManagerOld
{
     ...
     * @param string $root The root folder containing the <b>content</b> directory. This class
     * requires the <b>content</b> directory to exist and there be a particular directory layout
     * within that folder.
...
/*
    ...
    function reindex(): void {
        $categoryIndex = $this->buildCategoryIndex();
...
//TODO: Refactor
    ...
    private function init(): void {
        $this->slugIndexFilepath = $this->root.DS.self::INDEX_DIR.DS.'slugindex.inx';
        $this->categoryToPostIndexFilepath = $this->root.DS.self::INDEX_DIR.DS.'cat2posts.inx';
        $this->pageIndexFilepath = $this->root.DS.self::INDEX_DIR.DS.'pages.inx';
        $this->postIndexFilepath = $this->root.DS.self::INDEX_DIR.DS.'posts.inx';
        $this->categoryIndexFilepath = $this->root.DS.self::INDEX_DIR.DS.'categories.inx';
        $this->tagIndexFilepath = $this->root.DS.self::INDEX_DIR.DS.'tags.inx';
        $this->tagToPostIndexFilepath = $this->root.DS.self::INDEX_DIR.DS.'tag2posts.inx';

        if (!\file_exists($this->slugIndexFilepath)) {
            if (!\is_dir($this->root.DS.self::INDEX_DIR)) {
                if (!\mkdir($this->root.DS.self::INDEX_DIR, MODE, true)) {
                    $tmp = $this->root.DS.self::INDEX_DIR; // @codeCoverageIgnore
                    throw new \RuntimeException("mkdir() failed [$tmp]"); // @codeCoverageIgnore
                }
            }
            $this->reindex();
        }
    ...
    private function fetchPageIndex(): array {
        static $index = [];
        if (empty($index)) {
            $pagesRootDir = $this->appRoot.DS.IndexManagerOld::PAGES_DIR;
    ...
    private function fetchCategoryIndex(): array {
        static $list = [];
        if (empty($list)) {
//            $list = $this->indexDirectory($this->root.DS.self::CATEGORIES_DIR.DS.'*'.CONTENT_FILE_EXT);
        }
        return $list;
    }
    ...
    private function fetchPostIndex(): array {
        static $list = [];
        if (empty($list)) {
            $list = $this->indexDirectory($this->appRoot.DS.IndexManagerOld::USER_DATA_DIR.DS.
    ...
    /**
     * Create an array (index) of all the files and folders within a directory.
...
//            $index[$elem->getKey()] = $elem;
    ...
    private function createElementClass(
        ...
        return Element::createFromData($key, $path, $section, $category, $type, $username, $date, $tags);
    }
}
